(() => {
  ...
  function updateGallery(
    ...
    galleryTrack.style.transform =
      `translate3d(-${galleryIndex * 100}%, 0, 0)`;
  ...
  function renderGallery() {
    ...
    propertyImages.forEach(
      (
        ...
        slide.alt =
          `${activeProperty.title} — image ${index + 1} of ${propertyImages.length}`;
  ...
  gallery.addEventListener(
    ...
    event => {
      ...
      */
      ...
      galleryTrack.style.transform =
        `translate3d(calc(-${galleryIndex * 100}% + ${deltaX}px), 0, 0)`;
    }
  );
  ...
  function populateProperty() {
    ...
    priceElement.innerHTML =
      formattedPrice.period
        ? `${formattedPrice.price} <small>${formattedPrice.period}</small>`
  ...
  enquiryButton.addEventListener(
    ...
    () => {
      ...
      /*
        Save the property information for the request form.
        We will wire the homepage to read this in the next step.
      */
